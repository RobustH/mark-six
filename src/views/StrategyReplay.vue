<template>
  <div class="replay-container">
    <el-card>
      <template #header>
        <div class="card-header">
          <div class="left">
            ...
            <el-select v-model="selectedStrategyId" placeholder="选择要模拟的策略" clearable
              style="width: 200px; margin-left: 15px;" @change="handleStrategyChange">
              <el-option v-for="s in strategiesStore.strategies" :key="s.id" :label="s.name" :value="s.id" />
            </el-select>
          ...
          <div class="control-group">
            ...
            <el-select v-model="selectedDataSource" placeholder="选择数据年份" style="width: 150px"
      ...
      <div v-loading="loading" class="replay-content">
        ...
        <div v-if="currentState && currentState.signal" class="signal-alert">
          <el-alert :title="`策略信号触发: 投注 ${currentState.signal.target}`"
            :type="currentState.signal.is_hit ? 'success' : 'warning'"
            :description="currentState.signal.is_hit ? '✅ 本期中奖 (HIT!)' : '❌ 本期未中 (MISS)'" show-icon :closable="false" />
        </div>

        <!-- Strategy Execution Details -->
        <el-card v-if="currentStrategyConfig && currentState && currentState.signal_evaluation"
          class="strategy-details-card">
          <template #header>
            <div class="card-header">
          ...
          <el-row :gutter="20">
            <el-col :span="6">
              <div class="stat-box">
                ...
                <div class="val"
                  :class="{ profit: currentState.accumulated_stats.profit > 0, loss: currentState.accumulated_stats.profit < 0 }">
        ...
        <el-card v-if="currentState && currentState.betting_status" class="betting-status-card"
            ...
            <el-descriptions-item label="下注结果 (Bet Result)">
              <template v-if="currentState.betting_status.last_result">
                ...
                <span style="margin-left: 10px;">
                  ...
                  {{ currentState.betting_status.last_result.profit > 0 ? '+' : '' }}{{
                    currentState.betting_status.last_result.profit }})
                </span>
              </template>
            ...
            <el-descriptions-item label="下期下注 (Next Bet)">
              <template v-if="currentState.betting_status.next_bet">
                ...
                <el-tag type="warning" size="small" style="margin-left: 5px;"
                  v-if="currentState.betting_status.next_bet.step > 0">
        ...
        <div v-if="currentState" class="period-info">
          ...
          <div class="balls">
            <div v-for="(n, i) in currentState.result.numbers" :key="i" class="ball-container">
              <div class="color-circle" :class="getColorClass(currentState.result.numbers_colors[i])">
                <span class="circle-number">{{ n }}</span>
              </div>
            </div>
            <div class="ball-separator">+</div>
            <div class="ball-container special-container">
              <div class="color-circle" :class="getColorClass(currentState.result.color)">
                <span class="circle-number special-number">{{ currentState.result.special }}</span>
              </div>
            </div>
          </div>

          <div class="attributes">
            <el-tag effect="dark" size="small">{{ getZodiacName(currentState.result.zodiac) }}</el-tag>
            <el-tag :type="getColorType(currentState.result.color)" effect="dark" size="small">{{
              getColorName(currentState.result.color) }}</el-tag>
            <el-tag :type="currentState.result.parity === 1 ? 'warning' : 'info'" size="small">
              {{ currentState.result.parity === 1 ? '单' : '双' }}
            </el-tag>
            <el-tag :type="currentState.result.size === 1 ? 'danger' : 'success'" size="small">
              {{ currentState.result.size === 1 ? '大' : '小' }}
            </el-tag>
          </div>
        </div>
    ...
    <el-drawer v-model="showHistory" title="历史订单记录 (Recent 100)" size="40%">
      <el-table :data="currentState?.history_orders || []" style="width: 100%" stripe border
...
const getColorClass = (c: number) => ['red', 'blue', 'green'][c];
...
.special-container {
  margin-left: 10px;
  /* Extra spacing for special ball */
}

.special-container .color-circle {
  width: 32px;
  height: 32px;
  border-width: 7px;
  transform: scale(1.2);
...
.special-number {
  font-size: 20px;
  color: #f56c6c;
  /* Match DataManagement.vue style for special */
}
